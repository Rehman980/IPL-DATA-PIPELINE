...
class BatsmanStats:
    ...
    def analyze(deliveries_df):
        return deliveries_df.withColumn("batsman_match_runs", 
                        ...
                        F.sum(F.when(F.col("wide_runs") == 0,1)).alias("balls_faced"),
                        ...
                        F.countDistinct(F.when((F.col("batsman_match_runs") == 0) & (F.col("batsman_match_dismissed") == 1), F.col("match_id"))).alias("ducks"),
                        F.countDistinct(F.when(F.col("batsman_match_runs").between(50,99), F.col("match_id"))).alias("half_centuries"),
                        F.countDistinct(F.when(F.col("batsman_match_runs") >= 100, F.col("match_id"))).alias("centuries"),
                    ).withColumn(
                        "strike_rate",
